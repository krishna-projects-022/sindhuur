...
const adminblog = require("./routes/admin/blogsRoute")

const Razorpay = require("./routes/RazorPay");

const photoRoutes = require("./routes/photos");

const { router: messagesRoutes, registerSocketIO } = require("./routes/messagesRoutes");
...
const directories = ['Uploads/avatars', 'Uploads/documents', 'Uploads/photos'];
...
const generateToken = (user, isAdmin) => {
  ...
  if (!JWT_SECRET) {
    console.error('JWT_SECRET not configured during token generation');
...
app.use("/api",adminblog);


// Socket.IO setup
const onlineUsers = new Map();
registerSocketIO(io, onlineUsers);
...
// Use a distinct model name so it doesn't conflict with User
...
const storeOTP = async (email) => {
  ...
  try {
    // Remove old OTPs for this email (avoid duplicates)
    ...
    await otpDoc.save();
    console.log(`OTP ${otp} stored for ${normalizedEmail}`);
    return otp;
  } catch (err) {
...
// ---------- /api/send-email-otp (use storeOTP above) ----------
app.post("/api/send-email-otp", async (req, res) => {
  ...
  try {
    ...
    // send email (your existing transporter)
    ...
    console.log(`OTP sent to ${email} (development: OTP logged to server)`);
...
// ---------- /api/verify-email-otp (validate against OTP collection) ----------
app.post("/api/verify-email-otp", async (req, res) => {
  ...
  try {
    ...
    // **If** a user document already exists for this email, mark its otp.verified true
    const existingUser = await User.findOne({ "personalInfo.email": normalizedEmail });
    if (existingUser) {
      existingUser.otp = existingUser.otp || {};
...
// New route: Create user profile
// POST /api/create-profile
// UPDATED: Create Profile endpoint with complete data handling
app.post("/api/create-profile", async (req, res) => {
  try {
    ...
    const {
      ...
      familyInfo,
      familyTree = "",
      hobbies = {},        // NEW: Step 6 data
      horoscope = {},      // NEW: Step 7 data  
      lifestyle = {},      // NEW: Step 8 data
      credentials,
      subscription = { current: "free", details: {}, history: [] },
    ...
    // Basic validation
    if (!personalInfo || !credentials) {
      return res.status(400).json({ error: "Personal information and credentials are required" });
    }

    const requiredPersonalFields = ["name", "email", "mobile", "gender", "lookingFor"];
    const missingPersonalFields = requiredPersonalFields.filter(field => !personalInfo[field]);
    
    if (missingPersonalFields.length > 0) {
      return res.status(400).json({
        error: `Missing required personal fields: ${missingPersonalFields.join(", ")}`
      });
    }
    ...
    if (!mobileRegex.test(personalInfo.mobile)) {
      return res.status(400).json({ error: "Invalid mobile number format" });
    }

    // Check OTP verification
    const normalizedEmail = personalInfo.email.toLowerCase();
    const otpDoc = await OTP.findOne({ email: normalizedEmail, verified: true }).sort({ createdAt: -1 });
    if (!otpDoc) {
    ...
    if (existingUser && existingUser.credentials?.password) {
      return res.status(400).json({
        error: "Email or mobile number already registered",
        details: "Please use different credentials or log in to your existing account."
      });
    }
    ...
    // UPDATED: Prepare complete user data with all form fields
    const userData = {
      ...
      demographics: {
        ...
        physicalStatus: demographics?.physicalStatus || "normal",
        maritalStatus: demographics?.maritalStatus || "never-married",
        numChildren: demographics?.numChildren || "0",
        childrenLivingTogether: demographics?.childrenLivingTogether || "",
        religion: demographics?.religion || "",
        community: demographics?.community || "",
        motherTongue: demographics?.motherTongue || "",
        subcaste: demographics?.subcaste || "",
        willingAnyCaste: demographics?.willingAnyCaste || false
      },
      // UPDATED: Enhanced professional info with education fields
      professionalInfo: {
        education: professionalInfo?.education || "",
        fieldOfStudy: professionalInfo?.fieldOfStudy || professionalInfo?.otherEducation || "",
        occupation: professionalInfo?.occupation || "",
        income: professionalInfo?.income || "",
        college: professionalInfo?.college || "",        // NEW: Step 8 field
        course: professionalInfo?.course || "",          // NEW: Step 8 field
        passingYear: professionalInfo?.passingYear || "" // NEW: Step 8 field
      },
      location: {
      ...
      familyInfo: {
        ...
        familyWealth: familyInfo?.familyWealth || ""
      },
      familyTree: familyTree,
      // NEW: Hobbies data from Step 6
      hobbies: hobbies,
      // NEW: Enhanced horoscope data from Step 7
      horoscope: {
      ...
      // NEW: Lifestyle data from Step 8
      lifestyle: {
        eatingHabits: lifestyle?.eatingHabits || "",
      ...
      notifications: [],
      registeredEvents: [],
      // In your existing /api/create-profile endpoint, update the hobbies structure:
hobbies: {
  music: {
    genre: hobbies?.music?.genre || hobbies?.musicGenre || "",
    activity: hobbies?.music?.activity || hobbies?.musicActivity || ""
  },
  reading: {
    type: hobbies?.reading?.type || hobbies?.readingType || "",
    language: hobbies?.reading?.language || hobbies?.readingLanguage || ""
  },
  movies: {
    genre: hobbies?.movies?.genre || hobbies?.movieGenre || "",
    medium: hobbies?.movies?.medium || hobbies?.movieMedium || ""
  },
  sports: hobbies?.sports || "",
  fitness: hobbies?.fitness || hobbies?.fitnessActivity || "",
  food: {
    cuisine: hobbies?.food?.cuisine || hobbies?.cuisine || "",
    cooking: hobbies?.food?.cooking || hobbies?.cook || "",
    diet: hobbies?.food?.diet || hobbies?.dietType || ""
  },
  languages: hobbies?.languages || hobbies?.spokenLanguages || ""
},
    };
    ...
    if (existingUser) {
      ...
      savedUser = await User.findByIdAndUpdate(existingUser._id, userData, { new: true, runValidators: true });
    } else {
      // Create new user
    ...
    // Delete OTP after successful profile creation
    await OTP.deleteMany({ email: normalizedEmail });

    // Generate JWT token
    const token = jwt.sign(
      { 
        profileId: savedUser.profileId, 
    ...
    // UPDATED: Added redirect information
    res.status(201).json({
      ...
      user: {
        profileId: savedUser.profileId,
        email: savedUser.personalInfo.email,
        name: savedUser.personalInfo.name,
        role: savedUser.role,
        profileCreatedAt: savedUser.profileCreatedAt
      },
      redirectTo: "/login"  // NEW: Explicit redirect path
    });
...
app.get("/api/user-profile", async (req, res) => {
  console.log("Hit /api/user-profile in server.js");
  try {
    ...
    if (user.demographics?.dateOfBirth) {
      ...
      if (!isNaN(dob.getTime())) {
        age = Math.floor((new Date() - dob) / (1000 * 60 * 60 * 24 * 365));
    ...
    if (user.personalInfo?.profileImage) {
      image = user.personalInfo.profileImage.startsWith("http")
        ? user.personalInfo.profileImage
        : `${BASE_URL}/${user.personalInfo.profileImage.replace(/^\//, "")}`;
    }

    // FIX: Handle hobbies properly - it's now an object, not a string
    let hobbiesString = "Not specified";
    if (user.hobbies) {
      if (typeof user.hobbies === 'string') {
        hobbiesString = user.hobbies;
      } else if (typeof user.hobbies === 'object') {
        // Convert hobbies object to readable string
        const hobbyParts = [];
        if (user.hobbies.music?.genre) hobbyParts.push(`Music: ${user.hobbies.music.genre}`);
        if (user.hobbies.reading?.type) hobbyParts.push(`Reading: ${user.hobbies.reading.type}`);
        if (user.hobbies.movies?.genre) hobbyParts.push(`Movies: ${user.hobbies.movies.genre}`);
        if (user.hobbies.sports) hobbyParts.push(`Sports: ${user.hobbies.sports}`);
        if (user.hobbies.fitness) hobbyParts.push(`Fitness: ${user.hobbies.fitness}`);
        if (user.hobbies.food?.cuisine) hobbyParts.push(`Food: ${user.hobbies.food.cuisine}`);
        if (user.hobbies.languages) hobbyParts.push(`Languages: ${user.hobbies.languages}`);
        
        hobbiesString = hobbyParts.length > 0 ? hobbyParts.join(', ') : "Not specified";
      }
    }
    ...
    const profileData = {
      ...
      smoking: user.demographics?.smoking || user.lifestyle?.smoking || "Not specified",
      drinking: user.demographics?.drinking || user.lifestyle?.drinking || "Not specified",
      ...
      Status: user.personalInfo?.Status || "inactive",
      fatherOccupation: user.familyInfo?.father || "Not specified",
      motherOccupation: user.familyInfo?.mother || "Not specified",
      siblings: user.familyInfo?.siblings || "Not specified",
      profileComplete: user.personalInfo?.profileComplete || false,
      hobbies: hobbiesString, // FIX: Use the processed string instead of raw object
      ...
      personalInfo: {
        height: user.demographics?.height || "Not specified",
        maritalStatus: user.demographics?.maritalStatus || "Not specified",
        languages: user.demographics?.motherTongue ? [user.demographics.motherTongue] : [],
      },
      interests: hobbiesString !== "Not specified" ? [hobbiesString] : [], // FIX: Use hobbiesString
      bio: hobbiesString, // FIX: Use hobbiesString instead of raw object
      rating: "No rating",
      messages: 0,
      ...
      hobbiesData: user.hobbies, // Keep original hobbies object for reference
    };
...
// Fixed user-by-id endpoint
app.get("/api/user-by-id/:profileId", async (req, res) => {
  try {
    const { profileId } = req.params;
    console.log(`Fetching user by profileId: ${profileId}`);
    ...
    console.log("User found:", user.profileId);
    
    // FIX: Process hobbies to avoid split errors
    const processedUser = { ...user };
    if (processedUser.hobbies && typeof processedUser.hobbies !== 'string') {
      processedUser.hobbies = JSON.stringify(processedUser.hobbies);
    }
...
app.put("/api/update-profile", upload, async (req, res) => {
  try {
    let { profileId, updatedData: updatedDataString } = req.body;
    let updatedData = updatedDataString ? JSON.parse(updatedDataString) : req.body.updatedData;

    if (!profileId || !updatedData) {
      console.log("Validation failed:", { profileId, updatedData });
      return res.status(400).json({ error: "Profile ID and updated data are required" });
    }
    ...
    if (req.files && req.files.profileImage) {
      updateFields["personalInfo.profileImage"] = `/Uploads/avatars/${req.files.profileImage[0].filename}`;
      updateFields["personalInfo.avatar"] = `/Uploads/avatars/${req.files.profileImage[0].filename}`;
      console.log(`Updating profileImage to: ${updateFields["personalInfo.profileImage"]}`);
    }

    // Map fields as in the first route
    ...
    console.error("Detailed error updating profile:", error.stack);
    if (error.name === "ValidationError") {
...
// Simple update endpoint (fallback)
app.put("/api/update-profile-simple", async (req, res) => {
  try {
    ...
    console.log("Simple update for profile:", profileId);
    ...
    const updatedUser = await User.findOneAndUpdate(
      { profileId: profileId },
      { $set: updateObject },
      { new: true, runValidators: false } // Disable validators for simple update
    );
    ...
    const responseUser = {
      ...
      profileImage: updatedUser.personalInfo?.profileImage,
    ...
    console.error("Error in simple update:", error.message);
    res.status(500).json({ 
      error: error.message || "Failed to update profile",
      suggestion: "Please try the main update endpoint"
    });
  }
});

// Get user data for editing
app.get("/api/edit-profile/:profileId", async (req, res) => {
  try {
    const { profileId } = req.params;
    console.log(`Fetching edit profile data for: ${profileId}`);
    ...
    // Extract date components from dateOfBirth
    let birthDay = "", birthMonth = "", birthYear = "";
    if (user.demographics?.dateOfBirth) {
      try {
        const dob = new Date(user.demographics.dateOfBirth);
        birthDay = dob.getDate().toString().padStart(2, '0');
        birthMonth = (dob.getMonth() + 1).toString().padStart(2, '0');
    ...
    const editProfileData = {
      ...
      physicalStatus: user.demographics?.physicalStatus || "",
      maritalStatus: user.demographics?.maritalStatus || "",
      numChildren: user.demographics?.numChildren || "0",
      childrenLivingTogether: user.demographics?.childrenLivingTogether || "",

      // Background & Career
      religion: user.demographics?.religion || "",
      community: user.demographics?.community || "",
      subcaste: user.demographics?.subcaste || "",
      willingAnyCaste: user.demographics?.willingAnyCaste || false,
      education: user.professionalInfo?.education || "",
      fieldOfStudy: user.professionalInfo?.fieldOfStudy || "",
      otherEducation: "",
      ...
      familyStatus: user.familyInfo?.familyStatus || "",
      familyWealth: user.familyInfo?.familyWealth || "",
      about: user.personalInfo?.aboutMe || "",
      father: user.familyInfo?.father || "",
      mother: user.familyInfo?.mother || "",
      familyTree: user.familyTree || "",

      // Hobbies & Interests
      musicGenre: user.hobbies?.music?.genre || "",
      musicActivity: user.hobbies?.music?.activity || "",
      readingType: user.hobbies?.reading?.type || "",
      readingLanguage: user.hobbies?.reading?.language || "",
      movieGenre: user.hobbies?.movies?.genre || "",
      movieMedium: user.hobbies?.movies?.medium || "",
      sports: user.hobbies?.sports || "",
      fitnessActivity: user.hobbies?.fitness || "",
      cuisine: user.hobbies?.food?.cuisine || "",
      cook: user.hobbies?.food?.cooking || "",
      dietType: user.hobbies?.food?.diet || "",
      spokenLanguages: user.hobbies?.languages || "",

      // Horoscopic Details
      timeOfBirth: user.horoscope?.timeOfBirth || "",
      placeOfBirth: user.horoscope?.placeOfBirth || "",
      nakshatra: user.horoscope?.nakshatra || "",
      rashi: user.horoscope?.rashi || "",

      // Lifestyle & Education
      eatingHabits: user.lifestyle?.eatingHabits || "",
      smoking: user.lifestyle?.smoking || "",
      drinking: user.lifestyle?.drinking || "",
      college: user.professionalInfo?.college || "",
      course: user.professionalInfo?.course || "",
      passingYear: user.professionalInfo?.passingYear || "",

      // Profile Image
      profileImage: user.personalInfo?.profileImage || "https://via.placeholder.com/150",
    };
    ...
    console.error("Error retrieving edit profile:", error.message);
    res.status(500).json({ error: `Failed to fetch edit profile: ${error.message}` });
  }
});
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // Fallback for generateToken if not defined
    const token = typeof generateToken === 'function' 
      ? generateToken(user, false)
      : jwt.sign(
          { profileId: user.profileId, email: user.personalInfo.email, role: user.role },
          process.env.JWT_SECRET,
          { expiresIn: '7d' }
        );
    console.log(`Login successful for: ${normalizedEmail} (User)`, { profileId: user.profileId });
    res.json({
      token,
      profileId: user.profileId,
      role: user.role,
...
// Login route: /api/login (unchanged)
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    // Generate JWT
    const JWT_SECRET = process.env.JWT_SECRET; // Ensure SECRET_KEY is defined
    const tokenPayload = isAdmin
      ? { userId: user._id, email: user.email, role: user.role }
      : {
          profileId: user.profileId,
...
app.get("/api/profiles", async (req, res) => {
  try {
    const users = await User.find({ "otp.verified": true }).select(
      "profileId personalInfo.name personalInfo.gender personalInfo.profileImage demographics.dateOfBirth professionalInfo.occupation location.city location.state professionalInfo.education demographics.community demographics.religion professionalInfo.income horoscope.generated photos subscription.current status"
    );
    ...
    const profiles = users.map((user) => {
      ...
      let image = "https://www.shutterstock.com/image-vector/user-profile-icon-vector-avatar-600nw-2220431045.jpg?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80";
      if (user.personalInfo?.profileImage) {
        ...
        image = `https://your-image-base-url/${user.profileId}.jpg`;
      }
...
//       "profileId personalInfo.name personalInfo.email personalInfo.mobile personalInfo.gender personalInfo.lookingFor personalInfo.profileImage personalInfo.document demographics.dateOfBirth demographics.timeOfBirth demographics.placeOfBirth demographics.height demographics.maritalStatus demographics.religion demographics.smoking demographics.drinking demographics.community demographics.motherTongue professionalInfo.education professionalInfo.fieldOfStudy professionalInfo.occupation professionalInfo.income location.city location.state subscription.current familyInfo.father familyInfo.mother familyInfo.siblings hobbies horoscope profileCreatedAt appVersion personalInfo.Status"
//     ).lean();
//     if (!user) {
//       return res.status(404).json({ error: "User not found" });
//     }
...
//       maritalStatus: user.demographics?.maritalStatus || "Not specified",
//       religion: user.demographics?.religion || "Not specified",
//       smoking: user.demographics?.smoking || "Not specified",
//       drinking: user.demographics?.drinking || "Not specified",
//       community: user.demographics?.community || "Not specified",
//       motherTongue: user.demographics?.motherTongue || "Not specified",
//       education: user.professionalInfo?.education || "Not specified",
//       fieldOfStudy: user.professionalInfo?.fieldOfStudy || "Not specified",
...
//         languages: user.demographics?.motherTongue ? [user.demographics.motherTongue] : [],
//       },
//       interests: user.hobbies ? user.hobbies.split(", ") : [],
//       bio: user.hobbies || "No bio available",
//       rating: "No rating",
//       messages: 0,
//     };
